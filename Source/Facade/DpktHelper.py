...
class DPKTHelper(object):
    """
    A Façade for DPKT module.
    """

    def __init__(self):
        self.ssl_port = 443
        self.handshake_byte = 22
        self.reader = None

    def read_packet(self, packet):
        """
        Reads a packet.
        :param packet: Packet File ending in *.pcap
        :returns: Reader Object
        """
        try:
            pcap = open(packet, 'rb')
            self.reader = dpkt.pcap.Reader(packet)
        except FileNotFoundError:
            raise Exception(f'{packet} does not exist.')
        except OSError:
            raise Exception(f'{packet} is unable to be opened.')
        except ValueError as error:
            raise Exception(f'{packet} is not a valid PCAP: {error}')

    def get_server_hello_data(self, *ports):
        """
        Look through packet find traces of Server Hello Messages over ports provided.
        :param ports: Any number of ports to track, defaults to 443
        :return: List of Records containing:
            Source IP, Destination IP, Source Port, Destination Port, Version, and Selected CipherSuite
        """
        results = []
        for timestamp, buffer in self.reader:
            ...
            if not (tcp.dport == self.ssl_port or tcp.sport == self.ssl_port or ports) or len(tcp.data) <= 0:
                continue

            tls_handshake = bytearray(tcp.data)
            if tls_handshake[0] != self.handshake_byte:
                continue
...
# SSL Versions and Corresponding Hexcodes
...
# Ciphersuites and Corresponding Hexcodes
...
CIPHERSUITES = {
    ...
    0xff02 :	'TLS_RSA_WITH_28147_CNT_GOST94',
    0xff03 :	'TLS_RSA_WITH_28147_CNT_GOST94',
